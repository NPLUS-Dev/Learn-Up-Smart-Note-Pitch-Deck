/* =============================================================================
   Smart Note pitch deck — interactive feature demos (offline, dependency-free)
     · 교정 도우미      (proofreader: typo / grammar / LOGIC)      #demo-proof
     · AI 생성 엔진      (formula·graph·mermaid·3D·sim·image)        #demo-gen
     · 적응형 워크스페이스 (custom markdown + adaptive theme)        #demo-ws
     · 맥락형 AI 튜터    (note context + URL crawl)                  #demo-tutor
     · 3D 노트 우주 검색  (search → pulse)                           #demo-universe
     · 가져오기 (PDF heading split)                                  #demo-import
     · 시장 동심원 차트                                              #demo-market
...
(function () {
  ...
  /* ===========================================================================
     1) 교정 도우미 — beyond spellcheck: catches LOGIC errors in a paragraph
     ======================================================================== */
  function initProofreader() {
    ...
    var ISSUES = [
      { kind: "t", cls: "mark-typo",    sel: "#pf-1", was: "운동양", now: "운동량",
        msg: "<b>오타</b> — “<span class='was'>운동양</span>” → “<span class='now'>운동량</span>”" },
      { kind: "g", cls: "mark-grammar", sel: "#pf-2", was: "충돌 후에 의 총", now: "충돌 후의 총",
        msg: "<b>문법</b> — 조사 오류 “<span class='was'>충돌 후에 의</span>” → “<span class='now'>충돌 후의</span>”" },
      { kind: "l", cls: "mark-logic",   sel: "#pf-3", was: "마찰이 있어도 운동량은 항상 보존되므로", now: "외력(마찰)이 작용하면 운동량은 보존되지 않으므로",
        msg: "<b>논리 오류</b> — 외력이 있으면 운동량은 보존되지 않습니다. “<span class='was'>마찰이 있어도 항상 보존</span>” → “<span class='now'>외력이 있으면 보존되지 않음</span>”" },
      { kind: "l", cls: "mark-logic",   sel: "#pf-4", was: "가속도는 속도에 비례한다", now: "가속도는 알짜힘에 비례한다 (F = ma)",
        msg: "<b>논리 오류</b> — F=ma. 가속도는 속도가 아니라 <b>알짜힘</b>에 비례합니다. “<span class='was'>속도에 비례</span>” → “<span class='now'>알짜힘에 비례</span>”" }
    ];
    ...
    function reset() {
      ...
      list.innerHTML = "";
      if (count) count.innerHTML = "교정 실행을 눌러 문단을 분석하세요.";
    ...
    function scan() {
      ...
      if (count) count.innerHTML = "발견: 오타 <b class='t'>1</b> · 문법 <b class='g'>1</b> · <b class='l'>논리 오류 2</b>";
      list.innerHTML = "";
      ISSUES.forEach(function (iss, i) {
        var item = el("div", "proof__item",
          "<span class='proof__tag " + iss.kind + "'>" +
            (iss.kind === "t" ? "오타" : iss.kind === "g" ? "문법" : "논리") + "</span>" +
          "<span class='proof__msg'>" + iss.msg + "</span>" +
          "<button class='proof__accept'>수락</button>");
        ...
        accept.addEventListener("click", function () {
          ...
          accept.textContent = "반영됨 ✓"; accept.disabled = true;
        });
      });
  ...
  /* ===========================================================================
     2) AI 생성 엔진 + 플래너 — type a request → planner routes/splits → blocks
     ======================================================================== */
  function initGenerator() {
    ...
    var KIND_META = {
      formula:  { lab: "∑ 수식",         tip: "커서 위치에 삽입" },
      graph:    { lab: "📈 동적 그래프",  tip: "드래그 이동 · 휠 확대" },
      mermaid:  { lab: "🔧 구조도",       tip: "mermaid 코드 기반" },
      "3d":     { lab: "🧊 3D 오브젝트",  tip: "드래그 회전 · 휠 확대" },
      sim:      { lab: "🎚️ 시뮬레이션",   tip: "슬라이더 실시간 조작" },
      anim:     { lab: "🎞️ 2D 애니메이션", tip: "프레임 실시간 재생" },
      image:    { lab: "🎨 일러스트",     tip: "웹 검색 → 자체 재호스팅" }
    };

    // a simple traveling 2D wave with a dot riding it (the "간단한 2D 애니메이션" type)
    ...
    var MAKERS = {
      ...
      mermaid: function () {
        return block("mermaid",
          ...
          "<div class='note-mnode'>입력 이미지</div><div class='note-marrow'>↓</div>" +
          "<div class='note-mnode'>합성곱 (Conv)</div><div class='note-marrow'>↓</div>" +
          "<div class='note-mnode'>풀링 → 특징맵</div></div>");
      },
      "3d": function () {
      ...
      image: function () {
        ...
        b.insertAdjacentHTML("beforeend", "<div class='note-rehost'>🔒 웹 이미지를 <b>illustrations</b> 버킷에 재호스팅 — 원본이 삭제돼도 안전</div>");
        return b;
      }
    };
    ...
    btns.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var sh = el("div", "note-block note-busy", "<span class='spark'>✨</span> AI가 블록을 생성 중…");
  ...
  function makeChat(logEl) {
    return {
      add: function (who, html, opts) {
        ...
        var av = el("div", "msg__av", who === "user" ? "나" : "AI");
  ...
  /* ===========================================================================
     5) 맥락형 AI 튜터 — note context + live URL crawl, direct explain mode
     ======================================================================== */
  function initTutor() {
    var root = $("#demo-tutor");
    if (!root) return;
    ...
    var SCRIPT = {
      start: {
        text: "지금 <b>‘토크와 회전운동’</b> 노트를 쓰고 계시네요. 노트의 <span class='src'>τ = Iα</span> 식을 보니 — 얼마 전 배우신 <b>벡터 외적</b>과 바로 연결돼요! 더 볼까요?",
        opts: [
          { label: "오, 어떻게 연결되는데?", to: "connect" },
          { label: "사실 토크 자체가 아직 헷갈려", to: "gap" }
        ]
      },
      connect: {
        text: "벡터 외적 <span class='src'>τ = r × F</span> 을 성분으로 풀어보면 관성모멘트 I와 각가속도 α의 곱, 즉 <b>τ = Iα</b>가 그대로 나와요. 예전 노트에서 배운 개념이 이어지는 거예요.",
        opts: [{ label: "3D 시뮬레이션으로 보여줘 →", to: "gensim" }]
      },
      gensim: {
        text: "네! 텍스트 설명 대신, <b>r</b>과 <b>F</b>를 직접 조절할 수 있는 <b>3D 시뮬레이션</b>을 자발적으로 만들어 노트 커서 위치에 바로 넣었어요 — 두 슬라이더를 움직이면 <span class='src'>τ = r × F</span>와 회전 속도가 실시간으로 바뀌어요.",
        after: function (bubble, myGen) {
          ...
          var rRange = ctlRow("r (반지름)");
          var fRange = ctlRow("F (힘)");
          ...
          function applyRF() {
            ...
            readout.textContent = "🎚️ τ = r × F ≈ " + torque.toFixed(2) + " — 회전 속도(ω)가 실시간으로 바뀝니다";
            st3d.draw();
          }
        ...
        opts: [{ label: "이제 이해됐어, 고마워", to: "end" }]
      },
      gap: {
        text: "그럴 수 있어요 — 비슷한 걸 공부한 다른 학습자들도 처음엔 <b>‘힘이랑 뭐가 다른지’</b>에서 막히더라고요. 그 질문부터 답해드릴까요, 아니면 <b>단계별 학습 계획</b>을 바로 짜드릴까요?",
        opts: [
          { label: "학습 계획부터 짜줘", to: "plan" },
          { label: "막히는 질문부터 →", to: "connect" }
        ]
      },
      plan: {
        text: "좋아요 — <b>1주차: 힘 vs 토크</b> → <b>2주차: 관성모멘트</b> → <b>3주차: τ=Iα 응용</b> 순서로 짜봤어요. 승인하시면 학습 Planner에 바로 반영할게요. 👍",
        opts: []
      },
      end: {
        text: "좋아요, 필요할 때 또 불러주세요! 방금 대화는 프로필·이력·현재 노트 맥락을 전부 함께 참고해서 만든 답이었어요.",
        opts: []
      }
    };
  ...
  /* ===========================================================================
     6) 3D 노트 우주 — wire search box to the canvas pulse
     ======================================================================== */
  function initUniverseSearch() {
    ...
    function go() {
      ...
      if (tip) tip.textContent = hit ? "🔎 “" + hit.label + "” 로 이동" : (input.value ? "검색 결과 없음" : "🖱️ 드래그 회전 · 행성 호버");
    }
